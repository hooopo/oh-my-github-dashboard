...
class FetchFollowings
  ...
  def query(cusor)
    ...
    <<~GQL
      query {
        ...
        user(login: "#{login}") {
          users: following(first: 20 #{after}) {
            pageInfo {
              endCursor
              hasNextPage
            }
  ...
  def run
    cusor =  current_user.last_following_cursor
    ...
    loop do 
      ...
      if attrs.blank?
        puts "All followings synced successed"
        break
      else
        Following.upsert_all(attrs)
      ...
      current_user.update(last_following_cursor: cusor) if cusor.present?
  ...
  def fetch_data(cusor)
    q = query(cusor)
    puts "- Sync followings with cusor: #{cusor}"

...
class FetchRepos 
  ...
  def run
    cusor = current_user.last_repo_cursor
    ...
    loop do 
      ...
      if attrs.blank?
        puts "All repos synced successed"
        break
      else
        Repo.upsert_all(attrs)
      end
      cusor = end_cusor(data)
      
      remaining_count = remaining(data)
      is_has_next_page = has_next_page(data)
      current_user.update(last_repo_cursor: cusor) if cusor.present?
  ...
  def fetch_data(cusor)
    q = query(cusor)
    puts "- Sync repos with cusor: #{cusor}"
